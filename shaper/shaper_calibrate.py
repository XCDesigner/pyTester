...
import shaper_defs  as shaper_defs
...
class ShaperCalibrate:
    def __init__(self, server: Server) -> None:
        self.server = server
        self.error = server.command_error if server else Exception
        try:
            self.numpy = importlib.import_module('numpy')
        except ImportError:
            raise self.error("Failed to import `numpy` module")
    ...
    def background_process_exec(self, method, args):
        ...
        reactor = self.server.get_reactor()
        gcode = self.server.lookup_object("gcode")
    ...
    def fit_shaper(self, shaper_cfg, calibration_data, max_smoothing):
        ...
        for test_freq in test_freqs[::-1]:
            ...
            if max_smoothing and shaper_smoothing > max_smoothing and best_res:
                return best_res
            # Exact damping ratio of the machine is unknown, pessimizing
    ...
    def save_calibration_data(self, output: str, calibration_data: CalibrationData, shapers: Optional[List[CalibrationResult]] = None) -> None:
        try:
